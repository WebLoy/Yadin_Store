...
# --- INITIALIZATION (MASTER CLOUD LOAD) ---
if 'inventory' not in st.session_state:
    try:
        # Try loading everything from Google Sheets Master Brain
...
def update_credentials(new_user, new_pass, new_email):
    ...
    st.success("Credentials Auto-Saved! Please login again.")
            ...
            with st.form("add_form"):
                ...
                if st.form_submit_button("Save"):
                    if b in st.session_state.inventory['Barcode'].values: st.error("Barcode exists!")
                        ...
                        st.session_state.inventory = pd.concat([st.session_state.inventory, new_row], ignore_index=True); save_all(); st.success("Added & Cloud Synced!")
        with t3:
            if not st.session_state.inventory.empty:
                ...
                with st.form("edit_form"):
                    ...
                    if c_s.form_submit_button("💾 Save"):
                        ...
                        save_all(); st.success("Updated & Cloud Synced!"); st.rerun()
        ...
        with t5:
            ...
            if uploaded_backup is not None:
                if st.button("🔄 Confirm Upload & Overwrite"):
                    try:
                        ...
                        save_all()
                        st.success("✅ Restore Complete! Cloud Synced.")
                        st.rerun()
                    except: st.error("Invalid file.")
            
            st.divider(); st.subheader("📤 Manual Backup")
            ...
            with st.form("settings_form"):
                ...
                if st.form_submit_button("Save All Settings"):
                    ...
                    save_all(); st.success("Settings Saved & Synced!"); st.rerun()
